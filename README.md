...
| Junction（JCT） | DJセッションのメンバー・演奏順・接続状態を共有し、演奏を引き継ぐ |
...
- Junctionで別のDJが演奏中は、管理DJのPlayブラウザに単一の「Junction Live」が表示されます。任意のデッキへドラッグすると、現在再生中の曲と先読み済みの次曲を使って、曲名・再生位置・波形を表示専用で追従します。実デッキ、ミキサー、履歴は変更せず、会場で鳴る音はJunction設定のProgram出力から独立して送ります。
...
JCTは複数のDJが一つのセッションへ参加し、演奏順と共有デッキを引き継ぐための機能です。参加者は表示名ではなくDJ名で扱い、任意のアイコンと色を設定できます。全員に同じ「DJ一覧」を表示し、招待を作った時点では相手を招待中の枠として追加します。返答を取り込んで接続すると、その枠が実際のDJ情報と接続状態に置き換わります。各行では参加待ち・接続済み・プレイ中などの状態と通信品質を確認できます。管理DJの接続操作は相手の行、参加DJ自身の操作は一覧の上の「あなたの接続」に表示します。接続は「①招待 → ②返答 → ③接続」の順で案内し、コピー後は次の操作へ進みます。「その他」メニューは項目選択・外側クリック・Escで閉じます。

セッションを作成したDJが管理を担当しますが、最初にプレイする必要はありません。管理DJは一覧をドラッグして予定順を変更し、接続済みの任意のDJを最初のプレイ担当に選べます。参加DJを選んだ場合は、そのDJのデッキの状態から始まります。開始前に「セッション設定」で会場への音声出力デバイスを反映してください。未設定のまま開始しようとすると理由を表示し、一覧の順番は変えません。セッション開始後も一覧から次のDJを選び、準備完了を確認して引き継ぎます。参加DJの「次を希望する」は希望の表示だけで、引き継ぎは管理DJが「次のDJにする」で始めます。確定前なら「引き継ぎを取消」で取り消せます。

![Junction のDJ一覧（説明用データ）](public/docs/junction-lobby.png)

音声はWebRTC / Opusで送り、管理DJの独立したProgram出力から遅延付きで配信します。ヘッドホンCUE・プライベート試聴はProgramへ混ぜません。共有デッキは現在のプレイ担当者が操作し、交代前の操作権を持つ入力は拒否します。
...
flowchart LR
  ...
  Engine <-.->|"音声・操作・曲データ（P2P）"| Other["別のplumdeck"]
...
flowchart TB
  ...
  List --> DD["デスクトップ ⇔ デスクトップ<br/>plumdeck方式<br/>曲の転送・デッキ状態の引き継ぎ・音声照合付きの切り替え"]
  List --> LD["スマホ ⇔ デスクトップ<br/>Lite方式<br/>演奏担当の音声をホストが会場へ中継"]
  List --> LL["スマホ ⇔ スマホ<br/>Lite方式"]
```
...
スマホとデスクトップの間は、1本のブラウザ互換WebRTC接続（Opusステレオ音声と`junction.control` DataChannel）を使います。スマホがホストならデスクトップのMASTER音声をスマホの会場出力へ送り、デスクトップがホストならスマホのMASTER音声をネイティブのProgram出力へ送ります。演奏担当でない参加者の送信トラックは停止し、担当変更と演奏希望はDataChannelで即時に伝えます。
...
アプリのMCP画面に表示されるURLでStreamable HTTPに接続します。通常の製品ビルドは `http://127.0.0.1:48123/mcp`、開発時は `http://127.0.0.1:8001/mcp` です。現在は85ツール（うちJunction 39ツール）を登録し、実際の一覧は起動中のサーバーで確認できます。

Junctionでは、状態・交換テキスト・音声機器・STUN/TURNの確認、セッション作成／参加、招待と承認、DJ順、開始と引き継ぎ、復旧、Program出力と録音、プライベート試聴、マイク、Junction Liveの表示専用デッキ割り当てまで操作できます。MCPサイドカーは起動ごとに生成する認証情報でTauriのループバック専用ブリッジへ接続し、画面と同じネイティブ音声エンジンだけを使います。一般の状態取得では交換テキスト、TURN資格情報、ローカルパス、アイコン画像を返しません。招待・返答・通知は、それを生成する明示的な操作の結果でだけ返します。

AIエージェントには、初回だけ `junction_prepare_engine`、続いて `junction_get_state` を呼び、状態とpeer IDを確認させます。招待・返答は非同期で準備されるため、作成後に `junction_get_exchange_text` を使って取得します。操作後も状態を再取得させてください。参加の承認・拒否、セッション開始、引き継ぎ、録音開始、退出、終了は実際の音声や参加者へ影響するため、対象と実行意図を明示して依頼します。Junction Liveのデッキ割り当ては波形モニターだけを変更し、実デッキ、ミキサー、履歴、Junction Program出力を変更しません。
