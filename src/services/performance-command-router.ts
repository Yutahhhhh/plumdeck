...
export function routePerformanceCommand(op: string, params: Record<string, unknown>, lease: JunctionLease | null = captureJunctionLease()): Record<string, unknown> {
  ...
  if (state?.turn) {
    ...
    const reason = state.turn.signal === 'outgoing' ? tailLockReason(op, params as {deck?: string; leader?: string}, state.turn.tailDecks) : '';
    if (reason) throw new Error(reason);
  } else if (state?.localPeerId !== state?.performerPeerId && !state?.localPrep) throw new Error('別のDJがプレイ中です。手元の試聴または引き継ぎをご利用ください。');
