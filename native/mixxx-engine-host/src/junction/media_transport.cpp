...
struct MediaTransport::Impl {
 ...
 PcmRing decoded{128,960,2},preCodec{64,960,2};PcmRing* source=nullptr;std::atomic<bool> drainSource{false},running{false};std::thread worker;std::mutex mutex;
 ...
 while(running.load()){
  ...
  if(ring&&drainSource.exchange(false,std::memory_order_acq_rel)){ring->drop();src_reset(src);opus_encoder_ctl(enc,OPUS_RESET_STATE);fill=0;holding=false;held={};sourceGeneration=~quint64(0);sourceEpoch=~quint64(0);}
...
void MediaTransport::restartProducer(PcmRing*r){{std::lock_guard lock(d->mutex);d->source=r;}d->drainSource.store(true,std::memory_order_release);}
